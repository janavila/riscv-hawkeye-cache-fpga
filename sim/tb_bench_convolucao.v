// =============================================================================
// tb_bench_convolucao.v
// -----------------------------------------------------------------------------
// Espelha o benchmark "Convolucao 256x256" do modelo em C:
//   out[y][x] = img[y-1][x] + img[y][x] + img[y+1][x]
//
// Imagem de 256x256 inteiros = 256 KB = 8x o tamanho da L2.
// Cada "pixel" processado gera 3 leituras (linha y-1, y, y+1), cada uma
// com PC proprio, pois no codigo real seriam 3 instrucoes de load distintas.
// =============================================================================

`timescale 1ns/1ps

module tb_bench_convolucao #(
    parameter TOTAL_ACCESSES = 50000,
    parameter REPORT_EVERY   = 5000
);

    localparam [31:0] BASE_CONV = 32'h2000_0000;

    localparam [31:0] PC_CONV_UP   = 32'h0000_2001;
    localparam [31:0] PC_CONV_MID  = 32'h0000_2002;
    localparam [31:0] PC_CONV_DOWN = 32'h0000_2003;

    localparam CONV_WIDTH  = 256;
    localparam CONV_HEIGHT = 256;
    ...
    // =========================================================================
    // Geracao do padrao de Convolucao
    // =========================================================================
    integer conv_phase;
    integer conv_x, conv_y;
    ...
    task choose_pattern;
        ...
        begin
            ...
            conv_x = ((idx / 3) % (CONV_WIDTH  - 2)) + 1;
            conv_y = ((idx / 3) / (CONV_WIDTH  - 2)) % (CONV_HEIGHT - 2) + 1;

            case (conv_phase)
                0: begin // linha y-1
                    out_addr = BASE_CONV + (((conv_y - 1) * CONV_WIDTH + conv_x) * 4);
                    out_pc   = PC_CONV_UP;
                end
                1: begin // linha y
                    out_addr = BASE_CONV + ((conv_y * CONV_WIDTH + conv_x) * 4);
                    out_pc   = PC_CONV_MID;
                end
                default: begin // linha y+1
                    out_addr = BASE_CONV + (((conv_y + 1) * CONV_WIDTH + conv_x) * 4);
                    out_pc   = PC_CONV_DOWN;
                end
            endcase
    ...
    initial begin
        ...
        $display("ECHO,BENCHMARK,convolucao");
        ...
        $display("ECHO,FINAL,BENCHMARK,convolucao");
